...
#Fix a/an instances
#re.findall("(?<![\w])[aA]+[n]* [\w-]+", text)	#find "a ___" (until the end of a word) that isn't preceded by the end of another word
#for article_pair in re.findall("(?<![\w])[aA]+[n]* [\w-]+", text):
#	word = article_pair[article_pair.index(" ")+1:]
#	text = re.sub(article_pair, referenced(word), text)
for article_pair in re.findall("(?<![\w])[aA]+[n]* [\w-]+", text):
	text = re.sub(article_pair, referenced(article_pair.split()[1]), text)

#Fix Capitalization issues
text = str.capitalize(text[0]) + text[1:]
for letter in re.finditer("(?<=[\.\?!] )[a-z]", text):
	text = text[:letter.start()] + str.capitalize(text[letter.start()]) + text[letter.end():]
